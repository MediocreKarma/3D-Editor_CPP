...
namespace ColorSchemes {
    ...
    static const MyArray<MyArray<int, 3>, ACCENTCOLOR_COUNT * 2> themeColors = {
        /*LIGHT MODE*/
        //RED
        {WHITE, BLACK, accentColors[0]},
        //BLUE
        {WHITE, BLACK, accentColors[1]},
        //PINK
        {WHITE, BLACK, accentColors[2]},
        //YELLOW
        {WHITE, BLACK, accentColors[3]},
        //GREEN
        {WHITE, BLACK, accentColors[4]},
        /*DARK MODE */
        //RED
        {BLACK, WHITE, accentColors[0]},
        //BLUE
        {BLACK, WHITE, accentColors[1]},
        //PINK
        {BLACK, WHITE, accentColors[2]},
        //YELLOW
        {BLACK, WHITE, accentColors[3]},
        //GREEN
        {BLACK, WHITE, accentColors[4]}
    };
...
namespace Language {
    ...
    static const MyArray<MyArray<MyArray<char, 32>, 2>, TEXT_SIZE> Text {
        ...
        {"Pozitie", "Position"},
        {"Obiect nou", "New mesh"},
        {"Editare obiect", "Edit mesh"},
        {"Reset transformari", "Reset transforms"},
        {"Sterge obiect", "Delete mesh"},
        {"Trecere transformare locala", "Switch to local transform"},
        {"Trecere transformare globala", "Switch to global transform"},
        {"Strat Z: ", "Layer Z: "},
        {"Anulare", "Discard"},
        {"Inapoi", "Back"},
        {"Punct", "Point"},
        {"Strat curent", "Current layer"},
        {"Generare", "Generate"},
        {"Cub", "Cube"},
        {"Con", "Cone"},
        {"Cilindru", "Cylinder"},
        ...
        {"Segmente", "Segments"},
        {"Inele", "Rings"},
        {"Salvare spatiu", "Save space"},
        {"Scrie calea de salvare:", "Enter save path:"},
        {"Deschidere spatiu", "Open space"},
        {"Scrie calea spatiului:", "Enter open path:"},
        {"Creator obiecte", "Object Creator"},
        {"Editeaza punct:", "Edit point:"}
    };
}
...
class DropdownButton : public TextButton {
    public:
        ...
        DropdownButton(const int& xCenter_, const int& yCenter_,
            ...
            m_listVisibility(false),
            m_list() {}

        void addOption(const char* p) {
            int yOffset = yCenter + yLen / 2;
            int option_yLen = m_listHeight / lenList;
            m_list[m_index] = TextButton(xCenter - xLen / 2 + m_listWidth / 2,
                                         yOffset + (2 * m_index + 1) * option_yLen / 2,
        ...
        void changeOptionText(int index, const char* p) {
            int yOffset = yCenter + yLen / 2;
            int option_yLen = m_listHeight / lenList;
            m_list[index] =  TextButton(xCenter - xLen / 2 + m_listWidth / 2,
                                        yOffset + (2 * index + 1) * option_yLen / 2,
                                        m_listWidth, option_yLen, p);
        }
